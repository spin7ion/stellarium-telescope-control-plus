...
void TelescopeClientDirectNexStar::telescopeShoot(int exposure)
{
	if (!isConnected())
			return;
	nexstar->sendShoot(exposure);
}
...
void TelescopeClientDirectNexStar::telescopeSetTime()
{
	...
	qInfo()<< "SetTime("
		  ...
		  << tz.offsetFromUtc(cdt)/3600 << ";" << (unsigned int)tz.hasDaylightTime()
		<<')';
	nexstar->sendTime(ct.hour(), ct.minute(), ct.second(),
		cd.month(), cd.day(), cd.year()%100,
		tz.offsetFromUtc(cdt)/3600, (unsigned int)tz.hasDaylightTime());
}
