...
void SlewDialog::createDialogContent()
{
	...
	connect(ui->pushButtonTime, SIGNAL(clicked()), this, SLOT(setTime()));
	connect(ui->pushButtonShoot, SIGNAL(clicked()), this, SLOT(shoot()));
...
void SlewDialog::shoot()
{
	auto telescope = currentTelescope();
	if (!telescope)
		return;

	telescope->telescopeShoot(ui->spinBoxExposure->value());
}

void SlewDialog::setTime()
{
	auto telescope = currentTelescope();
	if (!telescope)
		return;

	telescope->telescopeSetTime();
}
