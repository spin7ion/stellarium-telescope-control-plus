...
NexStarCommandShoot::NexStarCommandShoot(Server &server, unsigned int exp_int) : NexStarCommand(server)
{
	exp=exp_int;
}
...
bool NexStarCommandShoot::writeCommandToBuffer(char *&p,char *end)
{
	#ifdef DEBUG5
	char *b = p;
	#endif
	...
	*p++ = 'G';
	*p++ = NIBTOASCII ((exp>>16) & 0x0f);
	*p++ = NIBTOASCII ((exp>>8) & 0x0f);
	*p++ = NIBTOASCII ((exp>>4) & 0x0f);
	*p++ = NIBTOASCII (exp & 0x0f);
	*p = 0;
...
int NexStarCommandShoot::readAnswerFromBuffer(const char *&buff, const char *end) const
{
	if (buff >= end)
		return 0;
	...
	{
		#ifdef DEBUG4
		*log_file << Now() << "NexStarCommandShoot::readAnswerFromBuffer: set time ok"
	...
	{
		#ifdef DEBUG4
		*log_file << Now() << "NexStarCommandShoot::readAnswerFromBuffer: set time failed." << StelUtils::getEndLineChar();
		#endif
	}
	buff++;
	return 1;
...
void NexStarCommandShoot::print(QTextStream &o) const
{
	o << "NexStarCommandShoot(" << exp <<')';
}
